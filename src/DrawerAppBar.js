  ...
  const drawer = (
    <Box onClick={handleDrawerToggle} sx={{ textAlign: 'center' }}>
      <Typography variant="h6" sx={{ my: 2, color: '#487307', fontWeight: 'bold' }} display={'flex'} justifyContent={'center'} alignItems={'center'}>
        <GrassIcon fontSize="large" />HERBAL GARDEN
      </Typography>
      <Divider />
      <List>
        {navItems.map((item) => (
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <HideOnScroll {...props}>
        <AppBar
          component="nav"
          sx={{
            background: 'linear-gradient(90deg, rgba(0,150,136,1) 0%, rgba(0,105,92,1) 100%)',
        ...
        >
          <Toolbar>
            ...
            <Typography
              ...
              component="div"
              display={'flex'}
              sx={{ flexGrow: 1, display: { xs: 'none', sm: 'flex' }, alignItems: 'center', color: '#fff', fontWeight: 'bold' }}
            >
              <Box>
                <GrassIcon fontSize="large" sx={{ color: '#4CAF50' }} />
              </Box>
              HERBAL GARDEN
            </Typography>
